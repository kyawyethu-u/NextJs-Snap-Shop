...
const VariantImages = () => {
  ...
  return (
    <div>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>You can upload multiple 10 images at once</FormDescription>
              <FormControl>
        ...
        />
        <div className='flex gap-2 my-2 flew-wrap'>
          {fields.map((field,index)=>(
            <div key={index} className={cn('border-2 border-gray-300 rounded-md relative h-[100px] w-[100px]',
              field.url.startsWith("blob:") && "animate-pulse"
            )}>
              <Image src={field.url} alt={field.name} width={70} height={50} className='object-cover w-full h-full overflow-hidden'/>
              <Trash className='w-4 h-4 cursor-pointer absolute top-1 right-1 text-red-600' onClick={(e) => {
                e.preventDefault();
                remove(index);
              }}/>
            </div>
