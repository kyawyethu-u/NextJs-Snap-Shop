...
import { db } from "..";
import { emailVerificationToken, resetPasswordToken, twoFactorTokens, users } from "../schema";
...
export const generateEmailVerificationToken= async(email: string) =>{
    ...
    if(existingToken){
        await db.delete(emailVerificationToken).where(eq(
            emailVerificationToken.id,existingToken.id))
    };
...
export const generateResetPasswordToken = async(email: string) =>{
    ...
    if(existingToken){
        await db.delete(resetPasswordToken).where(eq(
            resetPasswordToken.id,existingToken.id
        ))
    };
...
//-----------------twofactorToken----------------------------------------
export const checkTwoFactorCodeByEmail = async(email:string) =>{
    try{
...
export const generateTwoFactorCode = async(email: string) =>{
    try{
        ...
        const expires = new Date(new Date().getTime() + 30 * 60 * 1000);
        const existingCode = await checkTwoFactorCodeByEmail(email)
        if(existingCode){
            await db.delete(twoFactorTokens).where(eq(twoFactorTokens.id,existingCode.id))
        }
        const twoFactorCode = await db.insert(twoFactorTokens).values({
            email,
            token: code,
            expires,
        }).returning()
